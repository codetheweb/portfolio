    ...
    <Link href={`/projects/${name.toLowerCase()}`}>
      <a className={styles.container} onMouseEnter={playVideoPreview} onMouseLeave={resetVideoPreview}>
        <div className={styles.imageContainer} style={{paddingBottom: alignImageWithBottom ? 0 : '1rem'}}>
          ...
          {video && <div className={`${styles.videoContainer} ${shouldVideoHaveShadow ? styles.withShadow : ''}`} style={{opacity: videoIsPlaying ? 1 : 0}}><video src={video} ref={videoRef} muted preload="auto"/></div>}
        </div>
          ...
          <div className={styles.tags}>
            {
              technologies.map(technology => (
                <div className={styles.tag} key={technology}>{technology}</div>
              ))
            }
          </div>
        </div>
